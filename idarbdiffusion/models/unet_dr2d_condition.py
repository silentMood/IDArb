...
@dataclass
class UNetDR2DConditionOutput(BaseOutput):
    """
...
class UNetDR2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def __init__(
        ...
        mid_block_type: Optional[str] = "UNetMidBlockDR2DCrossAttn",
        up_block_types: Tuple[str] = ("UpBlock2D", "CrossAttnUpBlockDR2D", "CrossAttnUpBlockDR2D", "CrossAttnUpBlockDR2D"),
        ...
        # custom DR2D attention block  
        elif mid_block_type == "UNetMidBlockDR2DCrossAttn":
            self.mid_block = UNetMidBlockDR2DCrossAttn(
                num_domains=num_domains,
                transformer_layers_per_block=transformer_layers_per_block[-1],
                in_channels=block_out_channels[-1],
                temb_channels=blocks_time_embed_dim,
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (CrossAttnDownBlock2D, CrossAttnDownBlockDR2D, DownBlock2D, CrossAttnUpBlock2D, CrossAttnUpBlockDR2D, UpBlock2D)):
    ...
    def forward(
    ...
    ) -> Union[UNetDR2DConditionOutput, Tuple]:
        r"""
        ...
        return UNetDR2DConditionOutput(sample=sample)

    @classmethod
    def from_pretrained_2d(
            cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]],
            camera_embedding_type: str, num_domains: int, sample_size: int,
            zero_init_conv_in: bool = True,
            projection_class_embeddings_input_dim: int=6, 
        ...
        config['attn1_cd'] = attn1_cd
        config['attn_mid_cv'] = attn_mid_cv
        ...
        config['mid_block_type'] = "UNetMidBlockDR2DCrossAttn"
